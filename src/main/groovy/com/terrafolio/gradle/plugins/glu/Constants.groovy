package com.terrafolio.gradle.plugins.glu

class Constants {
	def final static SERVICE = 'SERVICE'
	def final static STATUS = 'STATUS'
	def final static FABRIC = 'FABRIC'
	def final static LOGGER = 'LOGGER'
	def final static CONSOLE_URL = 'CONSOLE_URL'
	def final static POLLING_ACTION = 'POLLING_ACTION'
	def final static COMPLETE_ACTION = 'COMPLETE_ACTION'
	def final static POLLING_STATUS = 'POLLING_STATUS'
	
	def final static STATUS_RUNNING = 'RUNNING'
	def final static STATUS_COMPLETED = 'COMPLETED'
	def final static STATUS_FAILED = 'FAILED'
	
	def final static SUCCESS = false
	def final static FAILURE = true
}
